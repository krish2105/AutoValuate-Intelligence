...
| MVP polish sprint (2026-07-18) | (a) README surfaces the unresolved AGPL licensing status (decision stays with the owner; Stripe remains test-mode); (b) price-history accrual fixed — the corpus dedupe was silently discarding every re-sighting, so time-on-market/price-drop/sold-proxy analytics were impossible; `price_history.csv` now accrues per cron; (c) Listing pack card — recommended ask (negotiation corridor, clean-rounded) + deterministic Dubizzle-ready listing text with scan damage disclosed; (d) `eval/condition_benchmark.py` — blind human 0-100 grades vs the production scorer (Spearman/MAE/band agreement), honest no-op until ≥1 graded case, directional label under 10 | 35/35 ui E2E; accrual unit-verified |
